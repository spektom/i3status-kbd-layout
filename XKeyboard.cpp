// xkeyboard.cpp
// Implementation of a class to get keyboard layout information and change
// layouts Copyright (C) 2008 by Jay Bromley <jbromley@gmail.com>
//
...
XKeyboard::XKeyboard()
  ...
  _currentGroupNum =
      (_currentGroupNum != xkbState.group) ? xkbState.group : _currentGroupNum;
  accomodateGroupXkb();
}
...
Bool XKeyboard::initializeXkb() {
  ...
  int minor = XkbMinorVersion;
  int opCode;
  /*Bool status =*/XkbQueryExtension(_display, &opCode, &_baseEventCode,
                                     &_baseErrorCode, &major, &minor);
  ...
  if (kbdDescPtr->ctrls != NULL) {
    ...
    _groupCount = 0;
    while (_groupCount < XkbNumKbdGroups && groupSource[_groupCount] != None) {
      _groupCount++;
    }
  }
  ...
  for (int i = 0; i < groupNameCount; i++) {
    if (_groupNames[i].empty()) {
      ...
      std::cerr << "Group Name " << i + 1 << " is undefined, set to '" << name
                << "'!\n";
      _groupNames[i] = name;
    }
  }
...
int XKeyboard::groupLookup(int srcValue, StringVector fromText,
                           StringVector toText, int count) {
...
int XKeyboard::groupCount() const { return _groupCount; }

StringVector XKeyboard::groupNames() const { return _groupNames; }

StringVector XKeyboard::groupSymbols() const { return _symbolNames; }

StringVector XKeyboard::groupVariants() const { return _variantNames; }
...
XkbSymbolParser::~XkbSymbolParser() { _nonSymbols.clear(); }

void XkbSymbolParser::parse(const std::string &symbols,
                            StringVector &symbolList,
...
bool XkbSymbolParser::isXkbLayoutSymbol(const std::string &symbol) {
  StringVectorIter result =
      find(_nonSymbols.begin(), _nonSymbols.end(), symbol);
